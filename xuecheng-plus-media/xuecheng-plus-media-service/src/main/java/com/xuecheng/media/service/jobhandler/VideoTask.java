...
//一次处理视频数量不要超过 cpu 核心数
            mediaProcessList =
                    mediaFileProcessService.getMediaProcessList(shardIndex, shardTotal,
                            processors);
...
//抢占任务
                    boolean b =
                            mediaFileProcessService.startTask(taskId);
                    if (!b) {
                        return;
                    }
...
//开始处理视频
                        Mp4VideoUtil videoUtil = new
                                Mp4VideoUtil(ffmpegpath, originalFile.getAbsolutePath(),
...
//访问 url
                    ...
                    try {mediaFileService.addMediaFilesToMinIO(mp4File.getAbsolutePath(),
